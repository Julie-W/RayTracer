        ...
        while (in) {
            if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
                ...
                // Also look at demo.txt to get a sense of why things are done this way.
                    ...
                    if (validinput) {
                        mat4 transform = Transform::translate(values[0],values[1],values[2]);
                        rightmultiply(transform,transfstack);
                    }
                }
                else if (cmd == "scale") {
                    validinput = readvals(s,3,values); 
                    if (validinput) {
                        mat4 transform = Transform::scale(values[0],values[1],values[2]);
                        rightmultiply(transform,transfstack);
                    }
                }
                else if (cmd == "rotate") {
                    validinput = readvals(s,4,values); 
                    if (validinput) {
                        mat4 transform = mat4(Transform::rotate(values[3],vec3(values[0],values[1],values[2])));
                        rightmultiply(transform,transfstack);
                    }
                }
